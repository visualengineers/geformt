                    ...
                    for (var i = 0; i < recognizedGesturesKeys.length; i++) {
                        ...
                        if (pGestureId == rGestureId) {
                            ...
                            if (!model.online) {
                                ...
                                // online recognition
                                //console.debug(recognizedGesturesKeys);
                                //if more than one candidate --> wait for next input
                                if (recognizedGesturesKeys.length == 1){ // && recognitionContext.checkmodels[rGestureId].oneOptionProcessedSuccessfully()) {
...
//                                             
        ...
        /**
         * SiGeRecognizer recognizes online gestures for simple atomar gestures 
         * like LINE_S.
         * @class
         */
        function SiGeRecognizer(){
        
        }
        
        SiGeRecognizer.prototype = /*@lends*/ {  
            /**
             * Main function to compare performed gestures and atomar gestures.
             * @param {Object} performedGestures
             * @param {Object} candidateGestures
             * @return {Object} Structure: (gestureId => optionIndex)
             */
            recognize: function(performedGestures, candidateGestures){
                ...
                for (var i = 0; i < candidateGestureKeys.length; i++) {
                    ...
                    var optionKeys = Object.keys(candidate);
                    var options = candidateGestures[candidateId];
                    var templates = performedGestures[candidateId];
                    var directions = templates[0].directions;
                    
                    if(directions.length == 0){
                        //no motion, all gestures are possible, recognized atoms = 0
                        result[candidateId] = parseInt(Object.keys(options)[0]);
                        recAtomNum = 0;
                    }
                    else{ // motion performed
                        for (var o = 0; o < optionKeys.length; o++) {
                            var atomicGestures = options[optionKeys[o]];
                            for (var ag = 0; ag < atomicGestures.length; ag++) {
                                var atomicGestureVariants = atomicGestures[ag].templates;
                                for(var t = 0; t<atomicGestureVariants.length; t++){
                                      var tempDir = atomicGestureVariants[t].directions;
                                      var lTest;
                                      // fill up directions for right result
                                      if (directions.length < tempDir.length){
                                          var temp = directions.slice();
                                          while (temp.length != tempDir.length){
                                              temp.push(0);
                                          }
                                          lTest = this._levenshteinDistance(tempDir, temp);
                                          d = (lTest - (tempDir.length - directions.length));
                                          bestDis[candidateId] = d;
                                             
                                      }
                                      else if (directions.length == tempDir.length){
                                          lTest = this._levenshteinDistance(tempDir, directions);
                                          if(lTest == 0){  // alle atome der geste exakt erkannt
                                              recAllAtoms = true;
                                          }
                                          d = lTest;
                                          bestDis[candidateId] = d;
                ...
                for (var r = 0; r < candidateGestureKeys.length; r++) {
                    var id = candidateGestureKeys[r];
                    // niedrigste Dis => gewinnt
                    var bestFit = bestDis[id];
                    if(bestFit === b){
                        result[id] = parseInt(Object.keys(options)[0]);
                    }
                    else if (b == 0 && directions.length > 1)
                        excludeG.push(id);
                }
                console.log(directions);
                console.log(result);
                result.recAllAtoms = recAllAtoms;
                return result;
            },
            /**
             * Add same values to one.
             * @param {Array} directions                       
             * @return {Array} new directions           
             */
            _addSameValues: function(directions){
                var nDirections = [];
                nDirections.push(directions[0]);
                
                for(var d = 0; d < directions.length; d++){
                    var oldDir = directions[d];
                    var nextDir = directions[d+1]; 
                    if (typeof nextDir != 'undefined'){
                        if (oldDir !== nextDir){
                            nDirections.push(nextDir);    
                        }
                    }  
                }
                return nDirections;
            ...
            /**
             * Calculate distance between two lists of direction strings.
             * @param {Array} s1
             * @param {Array} s2
             * @return {Number} Distance direction strings.
             */
            _levenshteinDistance: function(s1, s2){
                ...
                for (var i = 1; i <= n; i++) {
                    ...
                    for (var j = 1; j <= m; j++) {
                        ...
                        if (i > 1 && j > 1 && s_i == s2[j - 2] && s1[i - 2] == t_j) {
        ...
        return {
            ...
            init: function(){
                ...
                this.Strategy.SiGeRecognizer = new SiGeRecognizer(); 
            ...
            recognize: function(recognitionContext){
                ...
                else if (Object.keys(staticRecognizerResult).length > 0) {
                ...
                if (!onlineRecognition) {
                    ...
                    // recognize online path gestures
                    if (Object.keys(atomicPathGestureCandidates).length > 0) {
                        var siger = this.Strategy.SiGeRecognizer.recognize(gesturePathes, atomicPathGestureCandidates);
                        var recAllAtoms = siger.recAllAtoms;
                        delete siger.recAllAtoms;
                        onlineRecognizerResult = siger;
                    }
                    if (Object.keys(onlineRecognizerResult).length > 0){
                        recognitionResult = onlineRecognizerResult;
                        var recognitionResultKeys = Object.keys(recognitionResult);
                    }
                    for (var r = 0; r < recognitionResultKeys.length; r++) {
                        var gestureId = recognitionResultKeys[r];
                        var j = recognitionResultKeys[0];
                        
                        if (recAllAtoms)
                            recognitionContext.checkmodels[gestureId].setAllAtomsOfOptionTrue(0);
                        //console.log(recognitionContext.checkmodels[gestureId].checkmodel);
                    }
                   
                     
                    // if all are recognized set checkmodeloption to true
                    //recognitionContext.checkmodels[gestureId].checkmodel.setAllAtomsOfOptionTrue();
                    //_recognitionProcessManager.excludeGestureFromRecognition(j)
    ...
    CheckModel.prototype = /*@lends*/ {
         ...
         * @param {Object} optionIdx
         */
        allAtomsProcessedSuccessfully: function(){
            return false;
        },
        /**
         * Set all values of all options except the option at the given position to false.
